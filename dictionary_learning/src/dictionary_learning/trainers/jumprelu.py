...
class JumpReluTrainer(nn.Module, SAETrainer):
    ...
    def __init__(
        ...
        self.optimizer = torch.optim.Adam(self.ae.parameters(), lr=lr, betas=(0.99, 0.999), eps=1e-8)
...
class JumpReluCoordinateTrainer(nn.Module, SAETrainer):
    """
    Trains a JumpReLU autoencoder.

    Note does not use learning rate or sparsity scheduling as in the paper.

    This trainer also uses coordinate descent to break down the problem in two convex searches
    """
    ...
    def __init__(
        ...
        # Purely for logging purposes
        self.dead_feature_threshold = 1_000_00
        self.num_tokens_since_fired = torch.zeros(dict_size, dtype=torch.long, device=device)
        self.dead_features = -1
        self.logging_parameters = ["dead_features"]
    ...
    def loss(self, x: torch.Tensor, step: int, logging=False, **_):
        ...
        coordinate_block_steps = 1

        loss = recon_loss if (step // coordinate_block_steps % 2) == 0 else sparsity_loss
